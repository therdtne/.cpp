...
{
  int sum = 0;
  for(int i=0; i < len; i++){ // should be i < len, not i < len+1
    sum += data[i];
  }
  return sum;
}
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0'; //not otherword[len+1]
   // delete [] word; // don's delete this?
  return otherword;
}
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
    ...
    delete [] data; // deletes dynamic allocation from line 52
  }
